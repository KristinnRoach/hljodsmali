...
export class SingleUseVoice {
  ...
  start(midiNote: number): void {
    ...
    const transposedRate = this.semitoneToRate(
      midiRate,
      settings.pitch.transposition
    );
    const tunedRate = this.semitoneToRate(
      transposedRate,
      settings.pitch.tuneOffset
    );
    this.source.playbackRate.value = tunedRate;

    this.source.start(
      0,
      settings.time.startPoint,
      this.source.loop
        ? undefined
        : settings.time.endPoint - settings.time.startPoint
    );
  ...
  triggerAttack(): void {
    ...
    this.voiceGain.gain.linearRampToValueAtTime(
      ...
      this.audioCtx.currentTime + settings.volume.attackTime
    );
  ...
  triggerRelease(): void {
    ...
    this.voiceGain.gain.linearRampToValueAtTime(
      ...
      this.audioCtx.currentTime +
        settings.volume.releaseTime / this.source.playbackRate.value
    );

    this.source.stop(
      this.audioCtx.currentTime + settings.volume.releaseTime + 1
    );
  }
