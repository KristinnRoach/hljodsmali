...
// import ResizableSlider from '../UI/ReactSlider';
...
// useEffect(() => {
//   console.log('controls effect, dep: [searchParams]');
//   console.log(getSelectedSamples(), getSelectedSamples().length);
// }, [searchParams]);
...
const SamplerControls: React.FC = () => {
  ...
  const handleSampleSettingsChange = useCallback(
    (sampleId: string, settings: Partial<SampleSettings>) => {
      updateSampleSettings(sampleId, settings);
    },
    [updateSampleSettings]
  );

  /* CHECK IF CALLBACK FUNCTIONS ARE NEEDED OR CAN BE HANDLED LIKE THE loopVolume INPUT */
  ...
  return (
    <>
      {getSelectedSamples().length > 0 && (
        <button onClick={handleSave}>Save All</button> // change to save selected
      )}
